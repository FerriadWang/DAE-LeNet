...
class DenoisingAutoencoder(object):
    ...
    def _build_model(self, n_features):
        ...
        self.W1_, self.bh1_, self.bv1_ = self._create_variables1(self.n_components)

        self._create_encode_layer()
        self._create_encode_layer1()

        self._create_decode_layer1()
    ...
    def _create_variables1(self, n_components):
        ...
        W1_ = tf.Variable(utils.xavier_init(self.n_components, 192, self.xavier_init), name='enc-w1')
        bh1_ = tf.Variable(tf.zeros([192]), name='hidden-bias1')
        bv1_ = tf.Variable(tf.zeros([self.n_components]), name='visible-bias1')

        return W1_, bh1_, bv1_
    ...
    def _create_encode_layer1(self):

        """ Create the encoding layer of the network.
        :return: self
        """

        with tf.name_scope("W_x_bh1"):
            if self.enc_act_func == 'sigmoid':
                self.encode1 = tf.nn.sigmoid(tf.matmul(self.encode, self.W1_) + self.bh1_)

            elif self.enc_act_func == 'tanh':
                self.encode1 = tf.nn.tanh(tf.matmul(self.encode, self.W1_) + self.bh1_)
            elif self.enc_act_func == 'relu':
                self.encode1 = tf.nn.relu(tf.matmul(self.encode, self.W1_) + self.bh1_)
    ...
    def _create_decode_layer(self):
        ...
        with tf.name_scope("Wg_y_bv"):
            if self.dec_act_func == 'sigmoid':
                self.decode = tf.nn.sigmoid(tf.matmul(self.decode1, tf.transpose(self.W_)) + self.bv_)

            elif self.dec_act_func == 'tanh':
                self.decode = tf.nn.tanh(tf.matmul(self.decode1, tf.transpose(self.W_)) + self.bv_)

            elif self.dec_act_func == 'none':
                self.decode = tf.matmul(self.decode1, tf.transpose(self.W_)) + self.bv_
            elif self.enc_act_func == 'relu':
                self.encode = tf.nn.relu(tf.matmul(self.decode1, self.W_) + self.bh_)

            else:
                self.decode = None

    def _create_decode_layer1(self):

        """ Create the decoding layer of the network.
        :return: self
        """

        with tf.name_scope("Wg_y_bv1"):
            if self.dec_act_func == 'sigmoid':
                self.decode1 = tf.nn.sigmoid(tf.matmul(self.encode1, tf.transpose(self.W1_)) + self.bv1_)

            elif self.dec_act_func == 'tanh':
                self.decode1 = tf.nn.tanh(tf.matmul(self.encode1, tf.transpose(self.W1_)) + self.bv1_)

            elif self.dec_act_func == 'none':
                self.decode1 = tf.matmul(self.encode1, tf.transpose(self.W1_)) + self.bv1_
            elif self.enc_act_func == 'relu':
                self.encode1 = tf.nn.relu(tf.matmul(self.encode1, self.W1_) + self.bv1_)
    ...
    def transform_decode(self, data, name='train', save=False):
        """ Transform data according to the model.
        ...
        with tf.Session() as self.tf_session:
            ...
            if save:
                np.save(self.data_dir + self.model_name + '-decoded-' + name, decoded_data)

            return decoded_data

    def transform_W(self, data, name='W_', save=False):
        """ Transform data according to the model.
        ...
        with tf.Session() as self.tf_session:
            ...
            final_W_ = self.W_.eval({self.input_data_corr: data})

            if save:
                np.save(self.data_dir + self.model_name + '-decoded-' + name, final_W_)

            return final_W_

    def transform_b(self, data, name='b_', save=False):
        """ Transform data according to the model.
        ...
        with tf.Session() as self.tf_session:
            ...
            final_bv = self.bv.eval({self.input_data_corr: data})

            if save:
                np.save(self.data_dir + self.model_name + '-decoded-' + name, final_bv)

            return final_bv

    def transform_noise_data(self, data, name='train', save=False):
        """ Transform data according to the model.
        ...
        with tf.Session() as self.tf_session:
            ...
            noise_data = self.input_data_corr.eval({self.input_data_corr: data})

            if save:
                np.save(self.data_dir + self.model_name + '-noise-' + name, noise_data)
